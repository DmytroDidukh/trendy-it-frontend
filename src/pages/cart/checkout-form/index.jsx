...
const CheckoutForm = () => {
  const { cartItems, cartTotal, deliveryPrice } = useSelector(
    ({ Cart, Novaposhta }) => ({
      cartItems: Cart.list,
  ...
  const handleOnSubmit = () => {
    const isValidateCustomer = Object.values(customer).every(
      (val) => val.isValid
    );
    ...
    const isValidateAddress =
      deliveryMethod === 3
        ? Object.values(address).every((val) => val.isValid)
        : true;
    ...
    const cityToSend =
      deliveryMethod === 2
        ? delivery.city.value.split('_')[0]
        : deliveryMethod === 3
        ? address.city.value.split('_')[0]
        : null;
