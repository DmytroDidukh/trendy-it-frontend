...
const Delivery = ({
  ...
  const { cities, warehouses, streets, deliveryPrice, loading } = useSelector(
    ({ Novaposhta }) => ({
      ...
      {deliveryMethod === 2 && (
        <>
          {POST_DELIVERY_SELECT_DATA.map((data, i) => (
            <div key={i} className={'checkout-selection-dropdown'}>
              <Form.Field
                control={Select}
                options={i === 0 ? citiesOptions : warehousesOptions}
                label={{
                  children: data.label.children,
                ...
                searchInput={{
                  id: data.searchInput,
                  onChange: onTypeValueToDeliveryInput
                }}
                error={
              ...
              />
              {!!i && loading && (
                <Loader className='dropdown-loader' active inline />
              )}
            </div>
          ))}
        </>
      ...
      {!!deliveryPrice &&
        ...
        deliveryMethod && (
          <div className='delivery-price'>
            Ціна доставки: {deliveryPrice} UAH
          </div>
        )}
    </div>
  );
